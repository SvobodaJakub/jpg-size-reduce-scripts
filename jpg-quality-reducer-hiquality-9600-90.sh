...
if [[ ! -f reduced_quality.txt ]]; then
    ...
    nice -n 16 mogrify -define jpeg:q-table="${quantization_table}" -sampling-factor 4:2:2 -define jpeg:dct-method=float -quality "$quality" -resize "${resolution}"\> -filter Lanczos -interlace Plane -- *.[Jj][Pp]*[Gg] ;
    rm -f -- *.[Jj][Pp]*[Gg]~* ;
    for f in *.[Jj][Pp]*[Gg]
    do
        echo -n "${f}"
